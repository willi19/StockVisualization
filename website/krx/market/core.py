from website.krx.krxio import KrxWebIo, KrxFileIo, SrtWebIo
import pandas as pd
...
class MKD20009(KrxWebIo):
    @property
    def bld(self):
        return "MKD/03/0303/03030204/mkd03030204_03"
    
    def fetch(self, date, market, sector):
        """20009 GICS산업분류 산업별 종목현황
        :param date: 조회 일자(YYMMDD)
        ...
        :param sector: 조회 섹터
            ...
            3030 필수소비재 (가정및개인용품)
            35 건강관리
            3510 건강관리 (건강관리서비스및장비)
            3520 건강관리 (제약,생명공학및생명과학)
            40 금융
            4010 금융 (은행)
            4020 금융 (다각화된금융)
            4030 금융 (보험)
            45 정보기술
            4510 정보기술 (소프트웨이및IT서비스)
            4520 정보기술 (하드웨어및IT장비)
            4530 정보기술 (반도체및반도체장비)
            50 커뮤니케이션서비스
            5010 커뮤니케이션서비스 (통신서비스)
            5020 커뮤니케이션서비스 (미디어와엔터테인먼트)
            55 유틸리티
            5510 유틸리티 (유틸리티)
            60 부동산
            6010 부동산 (부동산)
            ...
            return: 
                    종목코드       종목명      현재가   대비  등락률(%)   매도호가   매수호가  거래량(주)     거래대금(원)    시가      고가     저가  액면가  상장주식수(주)   상장시가총액(원)
                0     060310           3S       2,365     -5       0.21      2,365      2,360     152,157     361,210,535    2,370     2,395    2,355     500      44,772,143    105,886,118,195
                1     095570    AJ네트웍스      5,400     70       1.31      5,400      5,380      90,129     485,098,680    5,330     5,470    5,260   1,000      46,822,295    252,840,393,000
                2     068400     AJ렌터카      12,000    400       3.45     12,050     12,000     219,282   2,611,434,750   11,600    12,000   11,550     500      22,146,300    265,755,600,000
                3     006840     AK홀딩스      55,000    800       1.48     55,200     55,000      16,541     901,619,600   54,700    55,300   53,600   5,000      13,247,561    728,615,855,000
                4     054620    APS홀딩스       4,475     10       0.22      4,475      4,460      31,950     142,780,675    4,440     4,520    4,440     500      20,394,221     91,264,138,975
        """
        result = self.post(date=date, mkt_tp_cd=market, gics_ind_grp_cd=sector,lang='ko')
